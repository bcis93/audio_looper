...
class TrackController:
    ...
    def tick(self):
        ...
        if self.state is self.State.idle:
            pass
        elif self.state is self.State.recording:
        ...
        elif self.state is self.State.waiting:
            pass
        elif self.state is self.State.overdub:
            self.track.continue_recording()
        else:
        ...
        # state update
        if self.state is self.State.idle:
            if self.button_pressed and Globals.waiting_to_start is 0:
            ...
            if self.button_pressed and Globals.waiting_to_start is 1:
    ...
    def start_button(self):
        if self.last_state is self.State.idle:
            self.state = self.State.idle
        elif self.last_state is self.State.recording:
            self.state = self.State.idle
        elif self.last_state is self.State.playing:
            self.track.start_playing()
            self.state = self.State.playing
        elif self.last_state is self.State.waiting:
            self.state = self.State.waiting
        elif self.last_state is self.State.overdub:
            self.track.start_playing()
            self.state = self.State.playing
        else:
            self.state = self.State.idle
    ...
    def is_recording(self):
        return True if self.state == self.State.recording else False

    def is_playing(self):
        return True if self.state == self.State.playing else False